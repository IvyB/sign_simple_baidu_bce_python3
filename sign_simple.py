...
def normalize_string(in_str, encoding_slash=True):
    ...
    in_str = in_str.encode(DEFAULT_ENCODING) if isinstance(
        in_str, str) else str(in_str)
...
def get_canonical_querystring(params):
    ...
    result = ['%s=%s' % (k, normalize_string(v)) for k, v in list(
...
def get_canonical_headers(headers, headers_to_sign=None):
    ...
    if headers_to_sign is None or len(headers_to_sign) == 0:
        headers_to_sign = {"host", "content-md5",
                           "content-length", "content-type"}

    # 对于header中的key，去掉前后的空白之后需要转化为小写
    # 对于header中的value，转化为str之后去掉前后的空白
    def f(key_value): return (
        key_value[0].strip().lower(), str(key_value[1]).strip())

    result = []
    for k, v in map(f, iter(headers.items())):
...
if __name__ == "__main__":
    credentials = BceCredentials(
        "0b0f67dfb88244b289b72b142befad0c", "bad522c2126a4618a8125f4b6cf6356f")
    http_method = "PUT"
